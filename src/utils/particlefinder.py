...
coefprime = xp.array([1,-8, 0, 8, -1])
coefsec = xp.array([-1, 16, -30, 16, -1])
...
class CrockerGrierFinder:
    ...
    def __init__(self, shape=(256,256), dtype=xp.float32):
        """Allocate memory once"""
        self.blurred = xp.empty(shape, dtype)
        self.background = xp.empty(shape, dtype)
        self.dilated = xp.empty_like(self.blurred)
        self.binary = xp.empty(self.blurred.shape, bool)
...
%s instead of %s"""%(image.shape, self.blurred.shape)
    ...
    def initialize_binary(self, maxedge=-1, threshold=None):
        if threshold is None:
            ...
            self.binary = (self.blurred>threshold) & (self.dilated==self.blurred)
        #eliminate blobs that are edges (in 2D only, not optimized)
        if self.blurred.ndim==2 and maxedge>0 :
            c0 = xp.asarray(xp.where(self.binary)).T
            #copy neighbourhoods
            ngbs = xp.zeros((3,)*self.blurred.ndim+(nb_centers,))
    ...
    def no_subpix(self):
        ...
        return asnumpy(xp.column_stack((vals, c0)))
    ...
    def subpix(self):
        ...
        if nb_centers==0 or self.binary.min():
            return np.zeros([0, self.blurred.ndim+1])
        centers = xp.empty([nb_centers, self.blurred.ndim+1])
        #original positions of the centers
        c0 = xp.asarray(xp.where(self.binary)).T
        #neighbourhood
        ngbs = xp.zeros((5,)*self.blurred.ndim+(nb_centers,))
        for index in xp.ndindex(ngbs.shape[:-1]):
            ps = c0 + xp.array(list(index))
            ngbs[index] = self.blurred[tuple(ps.T)]
...
class OctaveBlobFinder:
    ...
    def __init__(self, shape=(256,256), nbLayers=3, dtype=np.float32):
        """Allocate memory once"""
        self.layersG = xp.empty([nbLayers+3]+list(shape), dtype)
        self.layers = xp.empty([nbLayers+2]+list(shape), dtype)
        self.eroded = xp.empty_like(self.layers)
        self.binary = xp.empty(self.layers.shape, bool)
        self.time_fill = 0.0
        self.time_subpix = 0.0
        self.ncalls = 0
        self.noutputs = 0
        self.sizes = xp.empty(nbLayers)
...
%s instead of %s"""%(image.shape, self.layersG[0].shape)
        ...
        self.layersG[0] = xp.asarray(image)
    ...
    def initialize_binary(self, maxedge=-1, first_layer=False, maxDoG=None, minG=None):
        ...
        Centers are local minima of the DoG with negative value
            ...
            if minG is not None, the Gaussian layer avlue should be larger than minG.
        ...
        self.binary = (self.layers == self.eroded) & (self.layers<maxDoG) & (self.layers**2+1.0>1.0)
        if minG is not None:
            self.binary &= self.layersG[:-1]>minG
    ...
    def no_subpix(self):
        ...
        #original positions of the centers
        c0 = xp.array(xp.nonzero(self.binary)).T
    ...
    def subpix(self):
        ...
        for l in range(1,self.layers.shape[0]-1):
            ...
            cl = asnumpy(xp.array(xp.nonzero(self.binary[l][None])).T)
            cl[:,0] = l
            ...
            ngbs = xp.zeros((3,)+(2*self.sizes[l]+1,)*ndim+(len(cl),))
            for index in np.ndindex(ngbs.shape[:-1]):
            ...
            centers.append(asnumpy(centers_l))
        return np.vstack(centers)

        # for i, p in enumerate(c0):
        #     #neighbourhood, three pixels in the scale axis,
        #     #but according to scale in space
        #     r = self.sizes[int(p[0])]
        #     rv = xp.asarray([1]+[r]*(self.layers.ndim-1))
        #     ngb = xp.copy(self.layers[tuple(
        #         [slice(p[0]-1,p[0]+2)]+[
        ...
        #         ngb -= ped
        #     #center of mass
        #     centers[i,1:] = (xp.asarray(ndi.measurements.center_of_mass(
        #         ngb, labels, lab
        #         ))-rv)+p
    ...
    def __call__(self, image, k=1.6, maxedge=-1, first_layer=False, maxDoG=None, minG=None):
        """Locate bright blobs in an image with subpixel resolution.
Returns an array of (x, y, r, -intensity in scale space)"""
        self.ncalls += 1
        self.fill(image, k)
        self.initialize_binary(maxedge, first_layer, maxDoG, minG)
        t0 = time.process_time()
        centers = self.subpix()[:,::-1]
...
class MultiscaleBlobFinder:
    ...
    def __init__(self, shape=(256,256), nbLayers=3, nbOctaves=3, dtype=np.float32, Octave0=True):
        ...
        self.preblurred = xp.empty(shapes[0], dtype)
        self.octaves = [
    ...
    def __call__(self, image, k=1.6, Octave0=True, removeOverlap=True, maxedge=-1, deconvKernel=None, first_layer=False, maxDoG=None, minG=None):
        ...
        if Octave0:
            ...
            centers = [self.octaves[0](self.preblurred, k, maxedge, maxDoG=maxDoG, minG=minG)]
        else:
            centers = []
        if len(self.octaves)>1:
            if deconvKernel is not None:
                ...
                centers += [self.octaves[1](np.maximum(0, deconv), maxedge=maxedge, first_layer=first_layer, maxDoG=maxDoG, minG=minG)]
            else:
                centers += [self.octaves[1](ndi.gaussian_filter(xp.asarray(image), k), maxedge=maxedge, first_layer=first_layer, maxDoG=maxDoG, minG=minG)]
        ...
        for o, oc in enumerate(self.octaves[2:]):
            centers += [oc(
                ...
                k, maxedge, maxDoG=maxDoG, minG=minG,
                )]
